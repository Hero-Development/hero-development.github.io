  ...
  return (
    <AppearTransition>
      <div className=" p-6 overflow-y-auto " style={{ height: '82vh' }}>
        ...
        {selectedLog.length ? (
          selectedLog.map((log) => (
            <div key={log.id} className="pb-4 mt-2 text-xs border-b dark:border-gray-700">
              <p className="">{`[${dayjs(log.createdAt).format('YYYY-MM-DD HH:mm:ss.SSS Z')}]`} </p>
              <p className="italic">
                {log.method}
                {' - '}
                <span
              ...
              <p className="mt-1 break-all">Response: {JSON.stringify(log.data)}</p>
            </div>
          ))
        ) : (
