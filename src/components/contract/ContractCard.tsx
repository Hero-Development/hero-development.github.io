...
export const ContractCard = (props: any) => {
  ...
  const onClickTransaction = async (
    ...
    try {
      ...
      if (type === 'function') {
        ...
        response = await cont.methods[method](...Object.values(values))[funcType](option);
      }
      if (type === 'estimateGas') {
        console.log('GAS ESTIMATE EXECUTED');
        response = await cont.methods[method](...Object.values(values)).estimateGas(option);
      }
  ...
  return (
    <div className="flex flex-col justify-between transition duration-500 transform shadow dark:bg-gray-800 hover:shadow-md focus:shadow focus:outline-none rounded-xl">
      <p className={`px-3 text-sm  border-b dark:border-gray-700 rounded-t-md ${type.color}`}>
        {ele.name}
      </p>
        ...
        <div className="flex flex-col">
          <div className="flex items-center justify-between p-3">
            {type.type !== 'event' ? (
              <>
                ...
                >
                  {['pure', 'view'].includes(ele.stateMutability) ? 'Call' : 'Send'}
                </button>
                <button
                  onClick={handleSubmit((values) =>
          ...
          {contractResponse ? (
            <div className="w-full p-2 text-white bg-gray-700 dark:bg-gray-900 rounded-md">
              ...
              <p className="text-xs break-all ">{JSON.stringify(contractResponse)}</p>
            </div>
          ) : null}
        </div>
      </form>
